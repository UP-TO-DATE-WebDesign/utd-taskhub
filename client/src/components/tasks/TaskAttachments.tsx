...
const ACCEPT_ATTR =
	".jpg,.jpeg,.png,.webp,.gif,.pdf,.docx,.xlsx,.doc,.xls,.txt,.zip";
...
export function TaskAttachments({ projectId, taskId, canEdit }: Props) {
		...
		for (const f of Array.from(files)) {
			...
			if (f.size > MAX_BYTES) {
				toast.error("File too large", {
					description: `${f.name} exceeds 10 MB`,
				});
				continue;
			}
			valid.push(f);
	...
	async function handleDelete(att: TaskAttachment) {
		...
		try {
			await deleteTaskAttachment(projectId, taskId, att.id);
		...
		<div className="group relative aspect-[4/3] rounded-lg overflow-hidden border border-border bg-muted-subtle/40">
			...
			{canDelete && (
				<button
					...
					className="absolute top-1.5 right-1.5 h-6 w-6 rounded-full bg-black/60 text-white opacity-0 group-hover:opacity-100 hover:bg-danger transition-opacity flex items-center justify-center disabled:opacity-60"
				>
					{deleting ? (
