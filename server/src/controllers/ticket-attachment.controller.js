...
const ATTACHMENT_SELECT =
	"id, ticket_id, file_url, file_path, file_name, mime_type, file_size, uploaded_by, created_at";
...
async function fetchTicket(projectId, ticketId) {
...
export async function createAttachment(req, res, next) {
	try {
		...
		const objectPath = `${projectId}/${ticketId}/${Date.now()}-${safeName}`;

		const { error: uploadError } = await supabaseAdmin.storage
			.from(env.supabaseTicketAttachmentsBucket)
...
export async function deleteAttachment(req, res, next) {
	try {
		const { projectId, ticketId, attachmentId } = req.params;
		const ticket = await fetchTicket(projectId, ticketId);
		if (!ticket) {
			return res
				.status(404)
